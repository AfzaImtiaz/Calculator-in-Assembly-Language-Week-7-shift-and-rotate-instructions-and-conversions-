...
**MOD 3 logic:**
`TEST AX, 1` performs a non-destructive AND between `AX` and `1`. If bit 0 is zero (even), `ZF=1` and `JNZ` does not fire. If bit 0 is one (odd), `ZF=0` and `JNZ` fires to the odd label.
...
POP savedFlags
JNO Add_NoOvf      ; if OF=0, skip overflow warning
```
...
PUSHF
POP savedFlags
JNO Sub_NoOvf      ; overflow check (MOD 2)
...
PB_Loop:
    ...
    JC  PB_One      ; if CF=1, current bit is 1
    ; else print '0'
